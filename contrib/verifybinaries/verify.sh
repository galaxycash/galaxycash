...
###   galaxycashcore.org and galaxycash.org and compares them.
...
WORKINGDIR="/tmp/galaxycash_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://galaxy-hub.online"
HOST2="https://galaxy-hub.ru"
BASEDIR="/bin/"
VERSIONPREFIX="galaxycash-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'galaxycash-',
...
if [ $? -ne 0 ]; then
   echo "galaxycash.org failed to provide signature file, but galaxycashcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "galaxycash.org and galaxycashcore.org signature files were not equal?"
