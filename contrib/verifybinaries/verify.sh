...
###   bitcoincore.org and galaxycash.org and compares them.
...
if [ $? -ne 0 ]; then
   echo "galaxycash.org failed to provide signature file, but bitcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "galaxycash.org and bitcoincore.org signature files were not equal?"
