...
{
    QString ret = QString("galaxycash:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GCH unit input in lnReqAmount, we generate the URI with GCH as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(GalaxyCashUnits::format(GalaxyCashUnits::GCH, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
