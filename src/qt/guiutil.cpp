...
namespace GUIUtil
{
void SetGUIStyle(QWidget* widget)
{
    ...
    widget->setStyleSheet("QToolTip { color: #ffffff; background-color: #2a82da; border: 1px solid white; }");
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a GalaxyCash address (e.g. %1)").arg(QString::fromStdString(DummyAddress(Params()))));
#endif
...
{
    QString ret = QString("galaxycash:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "GalaxyCash.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "GalaxyCash (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("GalaxyCash (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "galaxycash.desktop";
    return GetAutostartDir() / strprintf("galaxycash-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=GalaxyCash\n";
        else
            optionFile << strprintf("Name=GalaxyCash (%s)\n", chain);
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (mask & check) {
            ...
            case NODE_BLOOM:
                ...
                break;
            case NODE_MASTERNODE:
                strList.append("MASTERNODE");
                break;
            default:
