...
#include "galaxycashaddressvalidator.h"
...
{
    widget->setMaxLength(GalaxyCashAddressValidator::MaxAddressLength);
    widget->setValidator(new GalaxyCashAddressValidator(parent));
    widget->setFont(galaxycashAddressFont());
}
...
bool parseGalaxyCashURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("galaxycash"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!GalaxyCashUnits::parse(GalaxyCashUnits::GCH, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseGalaxyCashURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("galaxycash://"))
    {
        uri.replace(0, 12, "galaxycash:");
    }
    QUrl uriInstance(uri);
    return parseGalaxyCashURI(uriInstance, out);
}
