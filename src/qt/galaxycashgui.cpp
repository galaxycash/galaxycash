                                                                                                                 ...
                                                                                                                 platformStyle(_platformStyle)
{
    ...
    progressBar->setStyleSheet("QProgressBar { background-color: #8C8C8C; text-align: center; color: white; border: 1px solid #4b4b4b; } QProgressBar::chunk { background: #001752; margin: 0px; }");
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a GalaxyCash address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and galaxycash: URIs)"));
    ...
    mintingAction = new QAction(QIcon(":/icons/minting"), tr("&Staking"), this);
...
//    connect(multisigAction, SIGNAL(triggered()), this, SLOT(gotoMultisigPage()));
    ...
    decryptForMintingAction = new QAction(QIcon(":/icons/lock_open"), tr("&Decrypt Wallet for Staking Only"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your GalaxyCash addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified GalaxyCash addresses"));
    ...
    openAction->setStatusTip(tr("Open a galaxycash: URI or payment request"));

    showHelpMessageAction = new QAction(QIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible GalaxyCash command-line options").arg(tr(PACKAGE_NAME)));

    openChatroomAction = new QAction(QIcon(":/icons/galaxycash"), tr("&Chatroom"), this);
    openChatroomAction->setStatusTip(tr("Open the GalaxyCash Discord in a web browser."));

    openForumAction = new QAction(QIcon(":/icons/galaxycash"), tr("&Forum"), this);
    openForumAction->setStatusTip(tr("Open https://talk.galaxycash.net in a web browser."));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to GalaxyCash network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitcoinGUI::updateHeadersSyncProgressLabel()
{
    ...
    if (estHeadersLeft > HEADER_HEIGHT_DELTA_SYNC) {
        double value = 100.0 / (headersTipHeight + estHeadersLeft) * headersTipHeight;
        progressBarLabel->setText(tr("Syncing Headers (%1%)...").arg(QString::number(value, 'f', 1)));
...
{
    QString strTitle = tr("GalaxyCash"); // default title
