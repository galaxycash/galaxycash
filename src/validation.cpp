...
const std::string strMessageMagic = "GalaxyCash Signed Message:\n";
...
static bool AcceptToMemoryPoolWithTime(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& tx, bool* pfMissingInputs, int64_t nAcceptTime, bool bypass_limits)
{
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransactionRef& tx, bool* pfMissingInputs, bool bypass_limits)
{
...
int GetInputAge(CTxIn& vin)
{
    ...
    CCoinsViewCache view(&viewDummy);
    {
        ...
        const Coin& coin = AccessByTxid(view, vin.prevout.hash);
...
{
    int nHalvings = nHeight / Params().GetConsensus().SubsidyHalvingInterval();
...
// galaxycash: miner's coin stake is rewarded based on coin age spent (coin-days)
int64_t GetProofOfStakeReward(int64_t nFees, int nHeight)
{
    if (chainActive.Tip() && chainActive.Tip()->nMoneySupply >= MAX_MONEY)
        return nFees;
    ...
    if (nHeight > Params().GetConsensus().LastPowBlock())
        return GetProofOfWorkReward(nFees, nHeight);
}

int64_t GetMNReward(int64_t nReward, int nHeight)
{
    return (int64_t)(nReward * 0.7);
}
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // galaxycash: we are using arguments that will have no effects in updateAlert()
...
static void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainTrust > chainActive.Tip()->nChainTrust + (GetBlockTrust(*GetLastBlockIndex(chainActive.Tip(), CBlockHeader::ALGO_X12, true)) * 6))) {
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase()) {
        ...
        if (fScriptChecks) {
            ...
            CSHA256().Write(scriptExecutionCacheNonce.begin(), 55 - sizeof(flags) - 32).Write((unsigned char*)&flags, sizeof(flags)).Finalize(hashCacheEntry.begin());
...
{
    RenameThread("galaxycash-scriptch");
...
bool CChainState::ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.IsProofOfStake()) {
        int64_t nBaseReward = GetProofOfStakeReward(nFees, pindex->nHeight);
        if (nBaseReward < nReward)
            return error("ConnectBlock(): %s block coinstake to large: %d < %d", block.GetHash().GetHex().c_str(), nBaseReward, nReward);
    }
    ...
    // galaxycash: coinbase reward check relocated to CheckBlock()
...
static void NotifyBlockTip()
{
    ...
    // Send block tip changed notifications without cs_main
    if (fNotify) {
        uiInterface.NotifyBlockTip(fInitialBlockDownload, pindexHeader);
    }
}
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature, bool fOldClient)
{
    ...
    // Size limits
    if (block.vtx.empty() || ::GetSerializeSize(block, SER_NETWORK, block.nVersion) > MAX_BLOCK_SIZE)
    ...
    // galaxycash: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // galaxycash: first coinbase output should be empty if proof-of-stake block
    ...
    if (fCheckSignature && block.IsProofOfStake() && !CheckBlockSignature(block))
...
bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    return g_chainstate.AcceptBlock(pblock, state, chainparams, ppindex, fRequested, dbp, fNewBlock, fCheckPoS);
}
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (block.nVersion < CBlockHeader::MINIMAL_VERSION)
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    if (::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > MAX_BLOCK_SIZE) {
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!CheckBlockHeader(block, state, chainparams.GetConsensus(), !fSetAsPos, fSetAsPos, fOldClient)) {
            if (fOldClient)
                fSetAsPos = (block.nNonce == 0);
            else {
    ...
    uiInterface.NotifyHeaderTip(IsInitialBlockDownload(), pindex);
...
{
    if (nFlags & CBlockIndex::BLOCK_MODIFIER_MAKED)
        return true;
    ...
    if (GetBlockHash() == Params().GetConsensus().hashGenesisBlock) {
        ...
        bnStakeModifier = 0;
        nStakeTime = 0;
        nFlags |= CBlockIndex::BLOCK_MODIFIER_MAKED;
    ...
    if (pprev && pprev->GetBlockHash() != Params().GetConsensus().hashGenesisBlock) {
        if (!pprev->IsValid(BLOCK_VALID_TRANSACTIONS))
            return true;

        CBlock prevBlock;
        if (!ReadBlockFromDisk(prevBlock, pprev, Params().GetConsensus())) {
            error("BlockProofOfStakeCheck(): *** ReadBlockFromDisk failed at %d, hash=%s", __func__, pprev->nHeight, pprev->GetBlockHash().ToString());
            return false;
        }
    ...
    if (IsValid(BLOCK_VALID_TRANSACTIONS)) {
        ...
        bool fGeneratedStakeModifierNew = false;
        if (!ComputeNextStakeModifier(pprev, nStakeModifierNew, fGeneratedStakeModifierNew))
            LogPrintf("AddToBlockIndex() : ComputeNextStakeModifier() failed \n");
        SetStakeModifier(nStakeModifierNew, fGeneratedStakeModifierNew);

        bnStakeModifier = ComputeStakeModifierV2(pprev, block.IsProofOfWork() ? block.GetPoWHash() : block.vtx[1]->vin[0].prevout.hash);
        ...
        if (block.IsProofOfStake() && !::CheckProofOfStake(pprev, block.nBits, *block.vtx[1], hashProofOfStake)) {
            return error("BlockProofOfStakeCheck() : CheckProofOfStake failed at %d, hash=%s", __func__, nHeight, GetBlockHash().ToString());
        }
        nFlags |= CBlockIndex::BLOCK_MODIFIER_MAKED;
        nStakeModifierChecksum = GetStakeModifierChecksum(this);
        if (!CheckStakeModifierCheckpoints(nHeight, nStakeModifierChecksum))
            return error("BlockProofOfStakeCheck() : CheckStakeModifierCheckpoints failed at %d, hash=%s", __func__, nHeight, GetBlockHash().ToString());
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    if (!fHasDevblock && fCheckPoS && pindex->IsProofOfStake() && !pindex->CheckProofOfStake(block)) {
    ...
    if (fHasDevblock && pindex) pindex->BuildStakeModifier(block);

    setDirtyBlockIndex.insert(pindex);

    if (pindexBestBlock == nullptr) {
        pindexBestBlock = pindex;
    } else if (pindex && pindexBestBlock) {
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fOldClient, bool fForceProcessing, bool* fNewBlock, CBlockIndex** ppindex, bool* fPoSDuplicate)
{
    ...
    CValidationState state;
    {
        if (ppindex == nullptr || *ppindex == nullptr) pindex = mapBlockIndex.count(pblock->GetHash()) ? mapBlockIndex[pblock->GetHash()] : nullptr;
        if (pindex && pindex->IsProofOfStake() != pblock->IsProofOfStake()) pindex->nFlags = pblock->IsProofOfStake() ? CBlockIndex::BLOCK_PROOF_OF_STAKE : 0;
        if (fNewBlock) *fNewBlock = false;
        ...
        bool ret = CheckBlock(*pblock, state, chainparams.GetConsensus(), !fDevblock, !fDevblock, !fDevblock, fOldClient);

        LOCK(cs_main);

        if (ret || fDevblock) ret = g_chainstate.AcceptBlock(pblock, state, chainparams, &pindex, fForceProcessing, nullptr, fNewBlock, (!fDevblock) && pblock->IsProofOfStake());
...
bool static IsDERSignature(const std::vector<unsigned char>& vchSig, bool haveHashType)
{
...
bool static IsLowDERSignature(const std::vector<unsigned char>& vchSig, bool haveHashType)
{
...
{
    size_t nMaxOrphanBlocksSize = gArgs.GetArg("-maxorphanblocksmib", 128) * ((size_t)1 << 20);
    while (nOrphanBlocksSize > nMaxOrphanBlocksSize) {
        // Pick a random orphan block.
        int pos = rand() % mapOrphanBlocksByPrev.size();
...
bool ProcessBlock(CNode* pfrom, const CBlock& block, bool fOldClient, bool fForceProcessing, bool* fNewBlock, CBlockIndex** ppindex, bool* fPoSDuplicate)
{
    AssertLockHeld(cs_main);
    ...
    CBlockIndex* pindex = mapBlockIndex.count(pblock->GetHash()) ? mapBlockIndex[pblock->GetHash()] : AddToBlockIndex(block, block.IsProofOfStake());
    if (fNewBlock) *fNewBlock = false;
    if (fPoSDuplicate) *fPoSDuplicate = false;
    ...
    // Check for duplicate
    uint256 hash = block.GetHash();
    if (mapBlockIndex.count(hash) && pindex->nStatus & BLOCK_HAVE_DATA) {
    ...
    if (!fReindex && !fImporting && pindex->nStatus & BLOCK_HAVE_DATA && block.IsProofOfStake() && setStakeSeen.count(block.GetProofOfStake()) && !mapOrphanBlocksByPrev.count(hash)) {
        if (fPoSDuplicate) *fPoSDuplicate = true;
        return error("ProcessBlock() : duplicate proof-of-stake (%s, %d) for block %s", block.GetProofOfStake().first.ToString(), block.GetProofOfStake().second, hash.ToString());
    }

    if (block.IsProofOfStake()) {
        if (!IsCanonicalBlockSignature(pblock.get(), false)) {
        ...
        if (!IsCanonicalBlockSignature(pblock.get(), true)) {
    ...
    if (pblock->GetHash() != Params().HashGenesisBlock() && pfrom != NULL) {
        ...
        BlockMap::iterator mi = mapBlockIndex.find(block.hashPrevBlock);
        if (mi == mapBlockIndex.end()) {
            g_connman->PushMessage(pfrom, CNetMsgMaker(pfrom->GetRecvVersion()).Make(NetMsgType::GETBLOCKS, chainActive.GetLocator(pindexBestBlock ? pindexBestBlock : chainActive.Genesis()), uint256(0)));
            return false;
        }
    }
    ...
    if (!mapBlockIndex.count(block.hashPrevBlock)) {
        LogPrintf("ProcessBlock: ORPHAN BLOCK %lu, prev=%s\n", (unsigned long)mapOrphanBlocks.size(), block.hashPrevBlock.ToString());
        ...
        if (pfrom) {
            ...
            if (block.IsProofOfStake()) {
                // Limited duplicity on stake: prevents block flood attack
                // Duplicate stake allowed only when there is orphan child block
                if (setStakeSeenOrphan.count(block.GetProofOfStake()) && !mapOrphanBlocksByPrev.count(hash))
                    return error("ProcessBlock() : duplicate proof-of-stake (%s, %d) for orphan block %s", block.GetProofOfStake().first.ToString(), block.GetProofOfStake().second, hash.ToString());
            }
            PruneOrphanBlocks();
            ...
            pblock2->stake = block.GetProofOfStake();
            ...
            if (block.IsProofOfStake())
                setStakeSeenOrphan.insert(block.GetProofOfStake());

            // Ask this guy to fill in what we're missing
            g_connman->PushMessage(pfrom, CNetMsgMaker(pfrom->GetRecvVersion()).Make(NetMsgType::GETBLOCKS, chainActive.GetLocator(chainActive.Tip()), GetOrphanRoot(hash)));
    ...
    // Store to disk
    if (!ProcessNewBlock(Params(), pblock, pfrom ? pfrom->GetRecvVersion() <= OLD_VERSION : false, fForceProcessing, fNewBlock, ppindex, fPoSDuplicate)) {
        return error("ProcessBlock() : ProcessNewBlock FAILED");
    }
    ...
    for (unsigned int i = 0; i < vWorkQueue.size(); i++) {
        ...
        for (std::multimap<uint256, COrphanBlock*>::iterator mi = mapOrphanBlocksByPrev.lower_bound(hashPrev);
            ...
            bool mutated;
            pblock->hashMerkleRoot = BlockMerkleRoot(block, &mutated);

            if (ProcessNewBlock(Params(), pblock, false, true, nullptr, nullptr, nullptr))
...
bool CChainState::LoadBlockIndex(const Consensus::Params& consensus_params, CBlockTreeDB& blocktree)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight) {
        ...
        // galaxycash: calculate stake modifier checksum
        pindex->nStakeModifierChecksum = GetStakeModifierChecksum(pindex);
...
#ifdef ENABLE_CHECKPOINTS
    // galaxycash: load hashSyncCheckpoint
...
bool LoadBlockIndex(const CChainParams& chainparams)
{
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // galaxycash: txindex is always enabled
...
// galaxycash: total coin age spent in transaction, in the unit of coin-days.
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee)
{
...

    {
        ...
        CAmount nValueIn = 0;
        {
            ...
            for (const CTxIn& txin : tx.vin) {
                if (AccessByTxid(view, txin.prevout.hash).IsSpent()) {
                    if (pfMissingInputs)
                        *pfMissingInputs = true;
                    return false;
                }
        ...
        PrecomputedTransactionData ptx(tx);
        if (!CheckInputs(tx, state, view, false, flags, true, true, ptx)) {
