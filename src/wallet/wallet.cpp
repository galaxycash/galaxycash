...
// galaxycash: optional setting to unlock wallet for block minting only;
...
int64_t CWalletTx::GetTxTime() const
{
    ...
    // galaxycash: we still have the timestamp, so use it to avoid confusion
    return tx->nTime;
}
...
// galaxycash: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (const auto& entry : mapWallet) {
            ...
            if (nSpendTime > 0 && pcoin->tx->nTime > nSpendTime)
                continue; // galaxycash: timestamp must not exceed spend time
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/galaxycash/galaxycash/pull/10340, but that change has been
    // postponed until after https://github.com/galaxycash/galaxycash/pull/10244 to
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fMasterNode && pcoin->tx->vout[i].nValue == MASTERNODE_COLLATERAL)
                    continue;

                if (!IsLockedCoin((*it).first, i) && !IsSpent((*it).first, i) && IsMine(pcoin->tx->vout[i]) && pcoin->tx->vout[i].nValue >= MIN_TXOUT_AMOUNT)
...
bool CWallet::SelectCoinsForStaking(const CAmount& nTargetValue, std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet) const
{
    std::vector<COutput> vCoins;
...
bool CWallet::CreateTransaction(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-galaxycash-address
            ...
            while (true) {
                ...
                // galaxycash: sub-cent change is moved to fee
 ...
 * https://galaxycashtalk.org/?topic=54527, or
 * https://github.com/galaxycash/galaxycash/pull/1393.
 */
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, int64_t nFees, int nHeight, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, CKey &key)
{
    ...
    for (const auto& pcoin : setCoins) {
        ...
        for (unsigned int n = 0; n < std::min(nSearchInterval, (int64_t)nMaxStakeSearchInterval) && !fKernelFound; n++) {
            ...
            if (CheckStakeKernelHash(chainActive.Tip(), nBits, header, txNew, *tx, prevoutStake, hashProofOfStake, true)) {
                // Found a kernel
                ...
                if (!Solver(scriptPubKeyKernel, whichType, vSolutions)) {
                    LogPrint(BCLog::STAKE, "CreateCoinStake : failed to parse kernel type=%d\n", whichType);
                    break;
                }
    ...
    int64_t nReward;
    {
        nReward = GetProofOfStakeReward(nFees, nHeight);
        nCredit += nReward;
    }
    ...
    //Masternode payment
    masternodePayments.FillBlockPayee(txNew, nFees, true);
    ...
    LogPrint(BCLog::STAKE, "Coin stake generated\n");
