...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    return CGalaxyCashAddress(keyID).ToString();
}


CGalaxyCashAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CGalaxyCashAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current GalaxyCash address for receiving payments to this account.");
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CGalaxyCashAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CGalaxyCashAddress& address = item.first;
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CGalaxyCashAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CGalaxyCashAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CGalaxyCashAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    ...
    CGalaxyCashAddress addr(strAddress);
    if (!addr.IsValid())
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    string strAccount = AccountFromValue(params[0]);
    CGalaxyCashAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid GalaxyCash address");
    int64_t nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    {
        CGalaxyCashAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid GalaxyCash address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    return CGalaxyCashAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CGalaxyCashAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CGalaxyCashAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CGalaxyCashAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; GalaxyCash server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
