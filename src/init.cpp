 ...
 * between galaxycashd, and galaxycash-qt and non-server tools.
*/
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("galaxycash-loadblk");
...
bool AppInitSanityChecks()
{
    ...
    // galaxycash: init hash seed
    galaxycashRandseed = GetRand(1 << 30);
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if galaxycash is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if galaxycash is started while in a temporary directory.\n",
    ...
    if (fMasterNode) {
        ...
        if (!strMasterNodePrivKey.empty()) {
            ...
            if (!MasternodeSetKey(strMasterNodePrivKey, errorMessage, key, pubkey)) {
