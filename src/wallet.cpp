...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CGalaxyCashAddress(redeemScript.GetID()).ToString();
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s GCH %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s GCH %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
// populate vCoins with vector of spendable COutputs
void CWallet::AvailableCoinsFrom(const CTxDestination &from, vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, AvailableCoinsType coin_type) const
{
    vCoins.clear();
...
bool CWallet::SelectCoins(int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type) const
{
    ...
    if (coin_type == ALL_COINS && fMasternodeSoftLock) {
        AvailableCoins(vCoins, true, coinControl, ONLY_NONDENOMINATED_NOT10000IFMN);
    }

    //if we're doing only denominated, we need to round up to the nearest .1 GCH
    if(coin_type == ONLY_DENOMINATED) {
        ...
        BOOST_FOREACH(int64_t v, anonSendDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1 GCH over
                ){
                    CTxIn vin = CTxIn(out.tx->GetHash(),out.i);
                    int rounds = GetInputAnonsendRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsFrom(const CTxDestination &from, int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type) const
{
    vector<COutput> vCoins;
    AvailableCoinsFrom(from, vCoins, true, coinControl, coin_type);
    ...
    // If coin control is not used and all coins were request we can activate the soft lock.
    if (coin_type == ALL_COINS && fMasternodeSoftLock) {
        AvailableCoinsFrom(from, vCoins, true, coinControl, ONLY_NONDENOMINATED_NOT10000IFMN);
    }
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, AvailableCoinsType coin_type)
{
    int64_t nValue = 0;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type))
                    return false;
...
bool CWallet::CreateTransactionSubfee(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, AvailableCoinsType coin_type)
{
    int64_t nValue = 0;
...
bool CWallet::CreateTransactionFrom(const CTxDestination &from, const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, AvailableCoinsType coin_type)
{
    int64_t nValue = 0;
    BOOST_FOREACH (const PAIRTYPE(CScript, int64_t)& s, vecSend)
    {
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (!SelectCoinsFrom(from, nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type))
                    return false;
...
bool CWallet::CreateTransactionFromSubfee(const CTxDestination &from, const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, AvailableCoinsType coin_type)
{
    int64_t nValue = 0;
    unsigned int nSubtractFee = 0;
    BOOST_FOREACH (const PAIRTYPE(CScript, int64_t)& s, vecSend)
...
bool CWallet::CreateTransactionSubfee(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, const AvailableCoinsType coinType)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransactionSubfee(vecSend, wtxNew, reservekey, nFeeRet, coinControl, coinType);
}
...
bool CWallet::CreateTransactionFrom(const CTxDestination &from, CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransactionFrom(from, vecSend, wtxNew, reservekey, nFeeRet, coinControl, ALL_COINS);
}
...
bool CWallet::CreateTransactionFromSubfee(const CTxDestination &from, CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransactionFromSubfee(from, vecSend, wtxNew, reservekey, nFeeRet, coinControl, ALL_COINS);
}
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CGalaxyCashAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CGalaxyCashAddress(address).ToString());
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s GCH %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s GCH %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
int64_t CWallet::GetAnonymizableBalance() const
{
...
int64_t CWallet::GetDenominatedBalance(bool unconfirmed) const
{
    if(fLiteMode) return 0;
...
int CWallet::GetRealInputAnonsendRounds(CTxIn in, int rounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        {
            LogPrint("anonsend", "GetInputAnonsendRounds INSERTING %s\n", hash.ToString());
        ...
        if(nout >= wtx->vout.size())
        {
            ...
            LogPrint("anonsend", "GetInputAnonsendRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
        ...
        BOOST_FOREACH(CTxIn in2, wtx->vin)
        {
            if(IsMine(in2))
            {
                int n = GetRealInputAnonsendRounds(in2, rounds+1);
...
int CWallet::GetInputAnonsendRounds(CTxIn in) const {
    LOCK(cs_wallet);
    int realAnonsendRounds = GetRealInputAnonsendRounds(in, 0);
    return realAnonsendRounds > nAnonsendRounds ? nAnonsendRounds : realAnonsendRounds;
}
...
string CWallet::PrepareAnonsendDenominate(int minRounds, int maxRounds)
{
    ...
    if(anonSendPool.GetState() != POOL_STATUS_ERROR && anonSendPool.GetState() != POOL_STATUS_SUCCESS)
        if(anonSendPool.GetEntriesCount() > 0)
    ...
    */
    ...
    while(nStep < nStepsMax) {
        ...
        BOOST_FOREACH(int64_t v, anonSendDenominations){
            ...
            if((anonSendPool.sessionDenom & (1 << 0)) && v == ((1000*COIN) +1000000)) {fAccepted = true;}
            else if((anonSendPool.sessionDenom & (1 << 1)) && v == ((100*COIN) +100000)) {fAccepted = true;}
            else if((anonSendPool.sessionDenom & (1 << 2)) && v == ((10*COIN)  +10000)) {fAccepted = true;}
            else if((anonSendPool.sessionDenom & (1 << 3)) && v == ((1*COIN)   +1000)) {fAccepted = true;}
            else if((anonSendPool.sessionDenom & (1 << 4)) && v == ((.1*COIN)  +100)) {fAccepted = true;}
    ...
    anonSendPool.SendAnonsendDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nAnonsendRoundsMin, int nAnonsendRoundsMax)
{
    vCoinsRet.clear();
    nValueRet = 0;
...
bool CWallet::SelectCoinsAnon(int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, int64_t& nValueRet, int nAnonsendRoundsMin, int nAnonsendRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nAnonsendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN : ONLY_DENOMINATED);
...
{
    return  nInputAmount != 0 && nInputAmount % ANONSEND_COLLATERAL == 0 && nInputAmount < ANONSEND_COLLATERAL * 5  && nInputAmount > ANONSEND_COLLATERAL;
}
