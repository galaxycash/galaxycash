    ...
    if (showTransaction(wtx)) {
        for (size_t nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
            ...
            if (wallet->IsMine(txOut)) {
                ...
                parts.push_back(KernelRecord(hash, nHeight, nTime, addrStr, txOut.nValue, nOut, wallet->IsSpent(hash, nOut), coinAge));
            }
        }
    }
...
double KernelRecord::getProbToMintStake(double difficulty, int timeOffset) const
{
    ...
    uint64_t coinAge = max(nValue * (chainActive.Tip() ? chainActive.Tip()->nHeight - nHeight : 1) / COIN, (int64_t)0);
