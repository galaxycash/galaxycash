...
// Copyright (c) 2011-2019 The GalaxyCash developers
...
public:
    ...
    CBigNum(signed char n) : self(NULL)
    {
        init();
        if (n >= 0)
            setulong(n);
    ...
    CBigNum(int64_t n) : self(NULL)
    {
        init();
        setint64(n);
    }
    CBigNum(unsigned char n) : self(NULL)
    {
        init();
        setulong(n);
    }
    CBigNum(unsigned short n) : self(NULL)
    {
        init();
        setulong(n);
    }
    CBigNum(unsigned int n) : self(NULL)
    {
        init();
        setulong(n);
    }
    CBigNum(uint64_t n) : self(NULL)
    {
        init();
        setuint64(n);
    }
    explicit CBigNum(uint256 n) : self(NULL)
    {
        init();
        setuint256(n);
    }
