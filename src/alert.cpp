...
{
    return AppliesTo(PROTOCOL_VERSION, FormatSubVersion(CLIENT_NAME, GALAXYCASH_VERSION, std::vector<std::string>()));
}
...
bool CAlert::CheckSignature(const CPubKey& alertKey) const
{
    if (!alertKey.Verify(Hash(vchMsg.begin(), vchMsg.end()), vchSig))
...
bool CAlert::ProcessAlert(const CPubKey& alertKey)
{
