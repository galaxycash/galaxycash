...
#else
    const char* pszModule = "galaxycash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GalaxyCash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GalaxyCash
    // Mac: ~/Library/Application Support/GalaxyCash
    // Unix: ~/.galaxycash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GalaxyCash";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "galaxycashd.pid"));
...
// Format the subversion field according to BIP 14 spec (https://en.galaxycash.it/wiki/BIP_0014)
