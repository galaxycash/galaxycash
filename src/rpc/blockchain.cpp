...
double GetDifficulty(const CChain& chain, const CBlockIndex* blockindex)
{
    if (blockindex == nullptr) {
        if (chain.Tip() == nullptr)
            ...
            blockindex = GetLastBlockIndex(chain.Tip(), CBlockHeader::ALGO_X12, false);
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.push_back(Pair("proofhash", blockindex->IsProofOfStake() ? blockindex->hashProofOfStake.GetHex() : blockindex->GetBlockHeader().GetPoWHash().GetHex()));
...
UniValue getdifficulty(const JSONRPCRequest& request)
{
    ...
    obj.push_back(Pair("proof-of-stake", GetDifficulty(GetLastBlockIndex(chainActive.Tip(), CBlockHeader::ALGO_X12, true))));
...
{
    return "    \"size\" : n,             (numeric) virtual transaction size as defined in BIP 141.\n"
...
UniValue getblockbyheight(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"strippedsize\" : n,    (numeric) The block size\n"
    ...
    if (nHeight < 0)
        nHeight = 0;
    else if (nHeight >= chainActive.Height())
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            CURRENCY_UNIT + "\n"
                            ...
                            "     \"addresses\" : [          (array of string) array of galaxycash addresses\n"
...
UniValue getmempoolinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"bytes\": xxxxx,              (numeric) Sum of all virtual transaction sizes as defined in BIP 141.\n"
...
UniValue getchaintxstats(const JSONRPCRequest& request)
{
    ...
    int blockcount = 30 * 24 * 60 * 60 / Params().GetConsensus().TargetSpacing(pindexBestHeader->nHeight); // By default: 1 month
...
static const CRPCCommand commands[] =
    {
        ...
        {"blockchain", "getblock", &getblockbyheight, {"height", "verbosity|verbose"}},
