...
UniValue getmasternodecount(const JSONRPCRequest& request)
{
    ...
    mnodeman.CountNetworks(GETHEADERS_VERSION, ipv4, ipv6, onion);

    obj.push_back(Pair("total", mnodeman.size()));
    obj.push_back(Pair("stable", mnodeman.stable_size()));
    obj.push_back(Pair("obfcompat", mnodeman.CountEnabled(GETHEADERS_VERSION)));
...
UniValue getmasternodestatus(const JSONRPCRequest& request)
{
    ...
    throw std::runtime_error("Masternode not found in the list of available masternodes. Current status: " + activeMasternode.GetStatus());
}
...
UniValue createmasternodebroadcast(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (strCommand != "alias" && strCommand != "all") || (strCommand == "alias" && request.params.size() < 2))
        throw std::runtime_error("createmasternodebroadcast \"command\" ( \"alias\")\n");
...
static const CRPCCommand commands[] =
    {
        ...
        {"masternode", "createmasternodebroadcast", &createmasternodebroadcast, {"command", "alias"}},
        {"masternode", "decodemasternodebroadcast", &decodemasternodebroadcast, {"hexstring"}},
        {"masternode", "relaymasternodebroadcast", &relaymasternodebroadcast, {}},
