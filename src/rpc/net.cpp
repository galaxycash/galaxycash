...
UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:8333\",  (string) The galaxycash server IP and port we're connected to\n"
...
UniValue getnetworkinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            CURRENCY_UNIT + "/kB\n"
                            "  \"incrementalfee\": x.xxxxxxxx,          (numeric) minimum fee increment for mempool limiting " +
            CURRENCY_UNIT + "/kB\n"
...
UniValue enforcecheckpoint(const JSONRPCRequest& request)
{
    ...
    if (Params().DevKey().IsValid() && !fEnforceCheckpoint)
        throw std::runtime_error(
